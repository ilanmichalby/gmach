...
import { Calendar as CalendarIcon, Check, ShoppingCart, ArrowLeft, ArrowRight, Package, CheckCircle2, Loader2, Search } from 'lucide-react';
import { cn } from '../lib/utils';
import { OPERATION_DAYS } from '../constants';
import { motion, AnimatePresence } from 'motion/react';

type Step = 'catalog' | 'dates' | 'items' | 'checkout' | 'success';
...
export function BookingFlow() {
  ...
  // Load availability when dates + step change
  useEffect(() => {
  ...
  const loadAvailability = async () => {
    ...
    try {
      ...
      const all = itemsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Item));
      const overlapping = await getOverlappingOrders(pickupDate, returnDate);
      const withAvailability = await Promise.all(all.map(async item => {
        const available = await calculateAvailableQuantity(item, pickupDate, returnDate, overlapping);
  ...
  const selectPickup = (date: Date) => {
    setPickupDate(date);
    if (returnDate && !isAfter(returnDate, date)) setReturnDate(null);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Stepper — only for booking steps */}
      {step !== 'catalog' && step !== 'success' && (
          ...
          {(['dates', 'items', 'checkout'] as Step[]).map((s, idx) => (
            <div key={s} className="flex items-center gap-3">
              <div className={cn(
                ...
                step === s
                  ? "bg-indigo-600 text-white shadow-xl shadow-indigo-200 scale-110 rotate-3"
                  : idx < ['dates', 'items', 'checkout'].indexOf(step)
                    ? "bg-emerald-500 text-white"
                    : "bg-white border border-slate-200 text-slate-400 font-normal"
              )}>
              ...
              {idx < 2 && (
                <div className={cn(
                  ...
                  idx < ['dates', 'items', 'checkout'].indexOf(step) ? "bg-emerald-200" : "bg-slate-100"
                )} />
              )}
            </div>
          ))}
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'catalog' && (
          ...
          >
            {/* Hero */}
            <div className="text-center py-10 bg-gradient-to-br from-indigo-50 to-white rounded-3xl border border-indigo-100">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-indigo-600 rounded-3xl text-white mb-4 shadow-lg shadow-indigo-200">
                <Package size={32} />
              </div>
              <h2 className="text-3xl font-black text-slate-900 mb-2">קטלוג הציוד</h2>
              <p className="text-slate-500 mb-6 max-w-md mx-auto">עיין בציוד הזמין לאירועים. כשמוכן — בחר תאריכים ותזמין.</p>
              <button
                onClick={goToBooking}
            ...
            {catalogLoading ? (
              <div className="text-center py-24">
                ...
                <p className="text-slate-400 text-sm font-bold tracking-widest uppercase">טוען קטלוג...</p>
              </div>
            ) : (
              <>
                <p className="text-sm text-slate-400 text-center">{filteredCatalogItems.length} פריטים</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {filteredCatalogItems.map(item => (
                    <div key={item.id} className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col">
                      <div className="aspect-[4/3] bg-slate-50 relative overflow-hidden">
                        {item.imageUrl ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center text-slate-200">
                            <Package size={56} strokeWidth={1} className="mb-1" />
                          </div>
                        )}
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-slate-400">
                            מלאי: <span className="font-bold text-slate-600">{item.totalQuantity}</span>
                          </span>
                          <button
                            onClick={goToBooking}
                            className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-4 py-2 rounded-xl text-sm font-bold hover:bg-indigo-600 hover:text-white transition-all"
                          >
                ...
                {filteredCatalogItems.length === 0 && (
                  <div className="text-center py-20 text-slate-400">לא נמצאו פריטים תואמים.</div>
                )}
                ...
                <div className="text-center pt-4">
                  <button
                    onClick={goToBooking}
                    className="inline-flex items-center gap-2 bg-indigo-600 text-white px-10 py-4 rounded-2xl font-bold text-lg hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100"
                  >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-right">
              <div>
                ...
                <input
                  ...
                  onChange={e => selectPickup(new Date(e.target.value + 'T12:00:00'))}
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setReturnDate(new Date(e.target.value + 'T12:00:00'))}
                />
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-12 justify-center">
              <button
                ...
                className="px-6 py-3 border border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"
              >
                <ArrowRight size={18} />
                חזרה לקטלוג
              </button>
              <button
                ...
                className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                          ...
                          <div className="flex items-center gap-3 bg-slate-50 rounded-2xl p-1.5 shadow-inner">
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (current < item.totalQuantity) addToCart(item, current + 1);
                              }}
